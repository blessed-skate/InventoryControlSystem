...
function doCreateCatUserWindow(){
	...
	cat_user_layout = cat_user_window.attachLayout({
		pattern: "1C",
		cells: [ {id: "a", text: "Usuarios", header: true} ]
	});
...
function createAdminUserAction(action, rId){
	...
	if(action==1){
		...
		cat_user_form.attachEvent("onButtonClick", function(name){
			cat_user_form.validate();
		});
	}else if(action==2){
		var listXMLString='<items>'
			+'<item type="settings" position="label-left" labelWidth="120" inputWidth="200" />'
			+'<item type="input" label="ID usuario" required="true" name="idUser" maxLength="100" validate="NotEmpty" value="'+cat_user_grid.cells(rId,0).getValue()+'"/>'
			+'<item type="input" label="Nombre usuario" required="true" name="username" maxLength="100" validate="NotEmpty" value="'+cat_user_grid.cells(rId,1).getValue()+'"/>'
			+'<item type="password" label="Contraseña" required="true" name="passUser" maxLength="100" validate="NotEmpty"/>'
			...
			+'</item>'
			+'<item type="input" label="Nombre(s)" required="true" name="nameUser" maxLength="100" validate="NotEmpty" value="'+cat_user_grid.cells(rId,3).getValue()+'"/>'
			+'<item type="input" label="Apellido(s)" required="true" name="lastnameUser" maxLength="100" validate="NotEmpty" value="'+cat_user_grid.cells(rId,4).getValue()+'"/>'
	...
	cat_user_form.attachEvent("onAfterValidate", function(status){
		if(status){
			if(action==1){
				dhtmlx.confirm({title:"",ok:"Si",cancel:"No",text:"Seguro?",
				...
				dhtmlx.confirm({title:"",ok:"Si",cancel:"No",text:"¿Esta seguro que desea eliminar el usuario?",
					callback:function(result){
						if(result){
			...
			showResponseXmlAlertError("<b>Error, validar que los campos esten correctos...</b>");
		}
	});
...
function createGrid(){
	...
	cat_user_grid.setHeader("Id, Nombre Usuario, Role, Nombre(s), Apellido(s), Sexo, Fecha Naciemiento, Fecha Registro, Fecha Ultima Actualizacion");
	cat_user_grid.attachHeader("#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter,#text_filter");
	cat_user_grid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro,ro");
	cat_user_grid.setInitWidths("50,80,100,100,120,100,100,80,80");
	cat_user_grid.setColSorting("str,str,str,str,str,str,str,str,str");
	cat_user_grid.setColAlign("left,left,left,left,left,letf,left,left,left");
