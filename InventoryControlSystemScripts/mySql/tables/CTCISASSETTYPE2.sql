DROP TABLE CISDB.CTCISASSETTYPE;

CREATE TABLE CISDB.CTCISASSETTYPE 
( 
  FIIDASSETTYPE INT(3) NOT NULL
, FCSUBCLASS VARCHAR(2) NOT NULL
, FCDESCRIPTION VARCHAR(100) NOT NULL 
, FDREGISTERDATE TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
, FDLASTUPDATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP 
, CONSTRAINT PRIMARY KEY ( FIIDASSETTYPE, FCSUBCLASS ) );

CREATE INDEX CTCISASSETTYPE_INDEX1 ON CISDB.CTCISASSETTYPE ( FIIDASSETTYPE, FCSUBCLASS ASC);